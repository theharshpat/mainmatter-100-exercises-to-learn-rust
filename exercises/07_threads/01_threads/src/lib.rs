...
pub fn sum(v: Vec<i32>) -> i32 {
    ...
    let first_half = Vec::from(&v[..n / 2]);
    let second_half = Vec::from(&v[n / 2..]);

    let handle1 = thread::spawn(move || first_half.into_iter().sum());
    let handle2 = thread::spawn(move || second_half.into_iter().sum());
